...
var Visualizer = function() {
    ...
    Polygon.prototype.drawPolygon = function() {
        ...
        if (val > 0) {
            ...
            if (val > 128) {
                r = (val-128)*110;
                g = ((Math.cos((2*val/128*Math.PI/2)- 4*Math.PI/3)+1)*110);
                b = (val-105)*0;
            }
            else if (val > 175) {
                r = (val-128)*0;
                ...
                r = ((Math.cos((2*val/128*Math.PI/2))+1)*10);
                g = ((Math.cos((2*val/128*Math.PI/2)- 4*Math.PI/3)+1)*10);
                b = ((Math.cos((2.4*val/128*Math.PI/2)- 2*Math.PI/3)+1)*0);
            }
            if (val > 210) {
    ...
    var drawBg = function() {
        ...
        grd.addColorStop(0.8, "rgba(" +
            Math.round(r) + ", " +
            Math.round(g) + ", " +
            Math.round(b) + ", 0.2)"); // edges are reddish
...
var SoundcloudLoader = function(player,uiUpdater) {
    var self = this;
    var client_id = "43fe1fb1c62286e98a16a73d2acda798"; // to get an ID go to http://developers.soundcloud.com/
    this.sound = {};
    this.streamUrl = "";
    this.errorMessage = "";
    this.player = player;
...
if(window.location.hash) {
  ...
  window.location.replace("http://hive.rafgeekee.com/#kflamer/big-data");
}
